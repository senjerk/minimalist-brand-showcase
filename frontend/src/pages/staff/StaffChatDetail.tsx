...
const ChatDetail = ({ 
  ...
  useEffect(() => {
    if (!id || id === 'unassigned' || id === 'assigned' || id === 'my') return;
    ...
    const wsUrl = `${wsProtocol}//${wsHost}/ws/support/chat/${id}/`;
  ...
  const canSendMessages = useMemo(() => {
    if (!chats || !userData) return false;
    const currentChat = chats.find(chat => chat.id.toString() === id);
    if (!currentChat) return false;
    
    // Проверяем, является ли пользователь ответственным за чат
    return currentChat.responsible_users?.some(user => user.id === userData.user_id) || false;
  }, [chats, id, userData]);
          ...
          <div className="border-t bg-background p-4">
            {canSendMessages ? (
              ...
              <div className="text-center py-4 text-muted-foreground">
                Присоединитесь к чату, чтобы отправлять сообщения
              </div>
            )}
          </div>
        </div>
